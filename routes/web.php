 ...
 * Returns: JSON with overall, contentScore, humanPct, aiPct, itemScores[1..25], and detail chips.
 */
Route::post('/analyze', function (Request $request) {
    ...
    try {
        ...
        // Fallback to file_get_contents (some servers block Guzzle)
        try {
    ...
    $h1s = $attrOfAll('//h1', 'innerText'); // will be empty (innerText not real DOM attr) but we’ll still count via nodes
    ...
    if ($scripts) {
        foreach ($scripts as $script) {
            ...
            if ($json === '') continue;
            $json = preg_replace('/\/\*.*?\*\//s', '', $json); // strip JS comments if any
            try {
    ...
    // Viewport presence (mobile-friendly)
    ...
    // Canonical ok if present and same host (or absolute)
    ...
    // Light penalty for very long title
    ...
    if ($varSent >= 6) $human += 10;           // varied sentence lengths
    if (preg_match('/\b(I|we|my|our|me|us)\b/i', $bodyText)) $human += 8; // human voice
    ...
    // 18. Mobile friendly (viewport meta)
    $item[18] = $scoreBool($hasViewport, 90, 55);

    // 19. Speed optimizations — proxy (few heavy images?) we use medium baseline
    $item[19] = $clamp(85 - max(0, $imgCount - 12) * 2, 55, 90);

    // 20. Core Web Vitals — unknown baseline
    $item[20] = 65;

    // 21. Clear CTAs — proxy via buttons / “contact / buy / sign up”
    $hasCTA = $exists('//button') || preg_match('/\b(contact|buy|shop|subscribe|sign up|download|get started|try now)\b/i', $bodyText);
    $item[21] = $scoreBool($hasCTA, 86, 62);

    // 22. Primary entity clearly defined (main keyword in first paragraph)
    $firstP = $textOf('(//p)[1]');
    $item[22] = $scoreBool($mainKeyword !== '' && Str::contains(Str::lower($firstP), $mainKeyword), 88, 60);

    // 23. Related entities covered — proxy via proper noun count
    $item[23] = $entityScore;

    // 24. Valid schema present (Article/Product/FAQ/WebPage etc.)
    $item[24] = $scoreBool($hasSchema && $hasArticleLike, 90, 60);

    // 25. Organization/sameAs present
    ...
    // "Content score": average of Content & Keywords (1–5) + Content Quality (10–13)
    ...
    return response()->json([
        ...
        // Checklist mapping (keys are numeric strings to be liberal)
