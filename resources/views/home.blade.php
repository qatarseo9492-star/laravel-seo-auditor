...
(function(){
  ...
  // Resolve to the final/canonical URL if the input redirects or declares a canonical/og:url
  async function resolveAnalyzeUrl(url){
    // Try to fetch HTML and honor canonical/og:url
    try{
      const html = await fetchRawHtml(url);
      if (html && html.length > 200){
        const d = (new DOMParser()).parseFromString(html, 'text/html');
        let cand = d.querySelector('link[rel="canonical"]')?.getAttribute('href') || 
                   d.querySelector('meta[property="og:url"]')?.getAttribute('content') || '';
        if (cand){
          // Resolve relative canonical against base
          try{
            const finalUrl = new URL(cand, url).href;
            return normalizeUrl(finalUrl) || url;
          }catch(_){ /* ignore */ }
        }
      }
    }catch(_){}
  ...
  /* ===== Smart main-content extraction + richer meta ===== */
  function smartExtractMain(html, baseUrl){
    var d=(new DOMParser()).parseFromString(html,'text/html');
    d.querySelectorAll('nav,header,footer,aside,menu,form,iframe,script,style,noscript,svg,canvas').forEach(function(x){ x.remove(); });
    var sels=['article','main','[role="main"]','#content','.article__content','.post','.entry-content','.post-content','.content','.page-content','.container'];
    var best=null;
    sels.forEach(function(sel){ d.querySelectorAll(sel).forEach(function(el){ var len=(el.textContent||'').replace(/\s+/g,' ').trim().length; if(len>(best&&best.len||0)) best={el:el,len:len}; }); });
    if(!best){
      var blocks=[];
      d.querySelectorAll('div,section').forEach(function(el){
        var text=(el.textContent||'').replace(/\s+/g,' ').trim();
        var pcl=el.querySelectorAll('p').length;
        if(pcl>=3 || text.length>800){ blocks.push({el:el,len:text.length}); }
      });
    ...
    var sample='';
    if(best && best.el){
      var nodes=[].slice.call(best.el.querySelectorAll('p,h1,h2,h3,li')).slice(0,60);
      sample=nodes.map(function(n){return n.textContent||'';}).join('\n\n');
    }else{
      sample=[].slice.call(d.querySelectorAll('p')).slice(0,14).map(function(p){return p.textContent||'';}).join('\n\n');
    }
    sample=(sample||'').replace(/\s{2,}/g,' ').trim();
    function pick(sel,attr){ var el=d.querySelector(sel); return el ? (attr ? (el.getAttribute(attr)||'').trim() : (el.textContent||'').trim()) : ''; }
    var title=(pick('title')||'');
    var metaDesc=(pick('meta[name=\"description\"]','content')||'');
    var canonical=(pick('link[rel=\"canonical\"]','href')||'').trim()||baseUrl;
    try{ canonical = new URL(canonical, baseUrl).href; }catch(_){}
    var robots=(pick('meta[name=\"robots\"]','content')||'n/a');
    var viewport=(pick('meta[name=\"viewport\"]','content')||'n/a');
    var origin=''; try{ origin=new URL(baseUrl).origin; }catch(_){}
    var internal=0; d.querySelectorAll('a[href]').forEach(function(a){ try{ var u=new URL(a.getAttribute('href'), baseUrl); if(!origin || u.origin===origin) internal++; }catch(_){} });
    var schema = !!(d.querySelector('script[type=\"application/ld+json\"]') || d.querySelector('[itemscope],[itemtype*=\"schema.org\"]'));
    var ogTitle=pick('meta[property=\"og:title\"]','content'); var ogDesc=pick('meta[property=\"og:description\"]','content');
    var twTitle=pick('meta[name=\"twitter:title\"]','content');  var twDesc=pick('meta[name=\"twitter:description\"]','content');
    var ld={};
    [].slice.call(d.querySelectorAll('script[type=\"application/ld+json\"]')).forEach(function(s){
      try{
        var j=JSON.parse(s.textContent||'{}'); var arr=Array.isArray(j)?j:[j];
        arr.forEach(function(x){
          if(x && typeof x==='object'){
            if((x['@type']||'').toString().toLowerCase().includes('article') || x.headline || x.name){
              ld.headline = ld.headline || x.headline || x.name || '';
            }
            if(x.author){
              var a=Array.isArray(x.author)? x.author[0] : x.author;
              ld.author = ld.author || (a && (a.name||a)) || '';
            }
            if(x.datePublished) ld.datePublished = ld.datePublished || x.datePublished;
          }
        });
      }catch(e){}
    });
    return {
      titleLen: title?title.length:null, metaLen: metaDesc?metaDesc.length:null, canonical: canonical, robots: robots,
      viewport: viewport, headings: d.querySelectorAll('h1,h2,h3').length, internalLinks: internal, schema: schema?'yes':'no',
      sampleText: sample, ogTitle: ogTitle, ogDescription: ogDesc, twTitle: twTitle, twDescription: twDesc,
      ldHeadline: ld.headline||'', ldAuthor: ld.author||'', ldDatePublished: ld.datePublished||''
    };
  }

  async function extractMetaFromHtmlV2(html, baseUrl){
    try{ return smartExtractMain(html, baseUrl); }catch(_){ return extractMetaFromHtml(html, baseUrl) || {}; }
  }

  async function fetchRobotsAndSitemap(url){
    try{
      var u=new URL(url); var origin=u.origin;
      var robots=''; var sitemap='';
      try{
        var r=await fetch('https://api.allorigins.win/raw?url='+encodeURIComponent(origin+'/robots.txt'), {cache:'no-store'});
        if(r.ok){ robots=await r.text(); }
      }catch(_){}
      var smCands=[]; if(robots){ var m=robots.match(/sitemap:\s*(.+)/i); if(m) smCands.push(m[1].trim()); }
      smCands.push(origin+'/sitemap.xml');
      for(var i=0;i<smCands.length;i++){
        try{
          var r2=await fetch('https://api.allorigins.win/raw?url='+encodeURIComponent(smCands[i]), {cache:'no-store'});
          if(r2.ok){ sitemap=smCands[i]; break; }
        }catch(_){}
      }
      return { robotsTxt: robots ? 'present' : 'missing', sitemap: sitemap||'' };
    }catch(_){ return { robotsTxt:'unknown', sitemap:'' }; }
  }
...
/* === Human vs AI rendering === */
  ...
  async function startSiteSpeed(url, strategy='mobile'){
    ...
    try{
      ...
      var inp = audits['interaction-to-next-paint']?.numericValue; // primary INP (ms)
      if (typeof inp!=='number') { inp = audits['experimental-interaction-to-next-paint']?.numericValue; } // older LH
      if (typeof inp!=='number') { inp = audits['total-blocking-time']?.numericValue; } // last-resort proxy (not INP)
  ...
  async async function analyze(){
    ...
    // Canonicalize/resolve input target
    url = await resolveAnalyzeUrl(url);
    if (input) { try { input.value = url; } catch(_){} }
    ...
    try{
      ...
      if(raw){
        var meta = await extractMetaFromHtmlV2(raw, url);
        data = mergeMeta(data, meta);
        if((!sample || sample.length<200) && meta.sampleText) sample = meta.sampleText;
        try{ var xtra=await fetchRobotsAndSitemap(url); data.robotsTxt=xtra.robotsTxt; data.sitemap=xtra.sitemap||data.sitemap; }catch(_){}
      }
    }catch(_){}
    ...
    setText('rRobotsTxt', (data.robotsTxt||'—'));
    setText('rSitemap', (data.sitemap||'—'));
