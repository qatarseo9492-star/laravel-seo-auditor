...
.hero-heading{font-size:3.8rem;font-weight:1000;line-height:1.02;margin:.1rem 0;letter-spacing:.8px;background:linear-gradient(90deg,#b892ff,#ff2045 55%,#ff8a5b 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;text-shadow:0 0 28px rgba(155,92,255,.25)}
...
/* Buttons */
.btn{--pad:.78rem 1.05rem;display:inline-flex;align-items:center;gap:.5rem;padding:var(--pad);border-radius:14px;border:1px solid transparent;cursor:pointer;font-weight:800;letter-spacing:.2px;transition:.2s}
...
.btn-pulse::after{content:"";position:absolute;inset:-2px;border-radius:14px;background:conic-gradient(from 0deg, rgba(61,226,255,.0), rgba(61,226,255,.25), rgba(155,92,255,.25), rgba(255,32,69,.25), rgba(61,226,255,.0));filter:blur(10px);opacity:.0;transition:.2s}
.btn-pulse:hover::after{opacity:.45}
...
.score-container{width:260px}
.score-wheel{width:100%;height:auto}
.score-wheel .ring-bg{fill:none;stroke:rgba(255,255,255,.12);stroke-width:14;stroke-linecap:round}
.score-wheel .ring-progress{fill:none;stroke:url(#gradBad);stroke-width:14;stroke-linecap:round;stroke-dasharray:339;stroke-dashoffset:339;filter:drop-shadow(0 0 12px rgba(155,92,255,.28))}
.score-wheel .inner-fill{stroke:none;fill:url(#fillBad)}
.score-wheel .glow{fill:none;stroke:rgba(255,255,255,.08);stroke-width:24;filter:blur(6px)}
.score-text{font-size:3.2rem;font-weight:1000;fill:#fff;text-shadow:0 0 18px rgba(255,32,69,.25)}
.score-text.good{fill:#a7f3d0}
.score-text.mid{fill:#fde68a}
.score-text.bad{fill:#fecaca}

/* Chips */
.chip{padding:.3rem .65rem;border-radius:999px;font-weight:800;background:rgba(155,92,255,.14);border:1px solid rgba(155,92,255,.28)}
.badge{display:inline-flex;align-items:center;gap:.4rem;padding:.35rem .6rem;border-radius:999px;border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.06);font-weight:900}
.badge .ico{width:20px;height:20px;display:inline-grid;place-items:center;border-radius:50%}
.badge.human{background:rgba(34,197,94,.14);border-color:rgba(34,197,94,.35)}
.badge.human .ico{background:rgba(34,197,94,.4)}
.badge.ai{background:rgba(245,158,11,.14);border-color:rgba(245,158,11,.35)}
.badge.ai .ico{background:rgba(245,158,11,.4)}
...
.checklist-item{
  ...
  position:relative;display:grid;grid-template-columns:1fr auto auto auto;gap:.6rem;align-items:center;
  padding:.75rem .8rem .75rem 1rem;border-radius:14px;border:1px solid rgba(255,255,255,.10);
...
.autoPulse{animation:selPulse .8s ease}
...
<!-- SVG gradients for the wheel (stroke + interior) -->
<svg width="0" height="0" aria-hidden="true">
  <defs>
    ...
    <!-- Interior radial fills -->
    <radialGradient id="fillGood" cx="50%" cy="50%" r="60%">
      <stop offset="0%" stop-color="rgba(34,197,94,.45)"/>
      <stop offset="100%" stop-color="rgba(34,197,94,.12)"/>
    </radialGradient>
    <radialGradient id="fillMid" cx="50%" cy="50%" r="60%">
      <stop offset="0%" stop-color="rgba(245,158,11,.45)"/>
      <stop offset="100%" stop-color="rgba(245,158,11,.12)"/>
    </radialGradient>
    <radialGradient id="fillBad" cx="50%" cy="50%" r="60%">
      <stop offset="0%" stop-color="rgba(239,68,68,.45)"/>
      <stop offset="100%" stop-color="rgba(239,68,68,.12)"/>
    </radialGradient>
  </defs>
</svg>
...
<div class="wrap">
  <header class="site">
    ...
    <div style="display:flex;gap:.5rem">
      <button class="btn btn-ghost btn-pulse" id="printTop"><i class="fa-solid fa-print"></i> <span data-i="print">Print</span></button>
    </div>
  </header>
    ...
    <div class="score-area">
      <div class="score-container">
        <svg class="score-wheel" viewBox="0 0 120 120" aria-label="Overall score">
          <!-- Interior fill (color changes) -->
          <circle class="inner-fill" cx="60" cy="60" r="44" fill="url(#fillBad)"/>
          <!-- Soft glow ring -->
          <circle class="glow" cx="60" cy="60" r="54"/>
          <!-- Rotated ring group for progress -->
          <g transform="rotate(-90,60,60)">
            <circle class="ring-bg" cx="60" cy="60" r="54"/>
            <circle class="ring-progress" id="ringProgress" cx="60" cy="60" r="54"/>
          </g>
          <!-- Big number -->
          <text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" class="score-text bad" id="overallScore">0%</text>
        </svg>
      </div>
      <div style="display:flex;flex-direction:column;gap:.5rem">
        <div style="display:flex;gap:.5rem;flex-wrap:wrap;align-items:center">
          <span class="chip">Overall: <b id="overallScoreInline">0</b>/100</span>
          <span class="badge human" id="humanBadge"><span class="ico"><i class="fa-solid fa-user"></i></span><span id="humanPct">—</span>% Human</span>
          <span class="badge ai" id="aiBadgeChip"><span class="ico"><i class="fa-solid fa-microchip"></i></span><span id="aiPct">—</span>% AI</span>
          <span class="badge" id="writerLabel">Writer: <b>—</b></span>
          <button id="viewAIText" class="btn btn-neon btn-pulse" style="--pad:.5rem .8rem"><i class="fa-solid fa-robot"></i> Evidence</button>
        </div>
    ...
    <div class="analyze-box" style="margin-top:12px;background:rgba(255,255,255,.02);border:1px solid rgba(255,255,255,.08);border-radius:16px;padding:14px">
      <form id="analyzeForm" class="analyze-form" onsubmit="return false;">
        ...
        <div class="analyze-row">
          <div></div>
          <button id="analyzeBtn" class="btn btn-danger btn-pulse" type="button"><i class="fa-solid fa-magnifying-glass"></i> <span data-i="analyze">Analyze</span></button>
          <button class="btn btn-neon btn-pulse" id="printChecklist" type="button"><i class="fa-solid fa-print"></i> <span data-i="print">Print</span></button>
          <button class="btn btn-ghost btn-pulse" id="resetChecklist" type="button"><i class="fa-solid fa-rotate"></i> <span data-i="reset">Reset</span></button>
        </div>
...
(function(){
  ...
  void main(){
    vec2 p=(uv-vec2(1.,1.)) * vec2(2.2, 2.0); // bottom-right emitter
    p.x += t*.15; p.y += t*.10;
    float q=f(p*1.6);
...
/* ---- Wheel helpers ---- */
const WHEEL={circumference:339, ring:null, text:null, fill:null};
function setScoreWheel(value){
  if(!WHEEL.ring){
    WHEEL.ring=document.getElementById('ringProgress');
    WHEEL.text=document.getElementById('overallScore');
    WHEEL.fill=document.querySelector('.inner-fill');
  }
  const v=Math.max(0,Math.min(100,Number(value)||0));
  const offset=WHEEL.circumference - (v/100)*WHEEL.circumference;
  WHEEL.ring.style.strokeDashoffset=offset;

  // Colors by zone
  let stroke='url(#gradBad)', fill='url(#fillBad)', tClass='bad';
  if(v>=80){ stroke='url(#gradGood)'; fill='url(#fillGood)'; tClass='good'; }
  else if(v>=60){ stroke='url(#gradMid)'; fill='url(#fillMid)'; tClass='mid'; }

  WHEEL.ring.setAttribute('stroke', stroke);
  WHEEL.fill.setAttribute('fill', fill);
  WHEEL.text.classList.remove('good','mid','bad'); WHEEL.text.classList.add(tClass);
...
(function(){
  ...
  document.getElementById('resetChecklist').addEventListener('click', ()=>{
    ...
    boxes().forEach(cb=>{cb.checked=false; cb.classList.remove('autoPulse');});
...
/* ---- Analyze: connect per-item scores & auto-select ≥80 ---- */
...
(function(){
  ...
  function setChecked(id,on){
    ...
    if(el){ el.checked=!!on; el.classList.toggle('autoPulse', !!on); }
  }
  ...
  async function analyze(){
    ...
    try{
      ...
      // Per-item scores + color accents + auto-select
      let autoCount=0;
      for(let i=1;i<=25;i++){
      ...
      document.dispatchEvent(new Event('change')); // update progress bar/totals

      // Overall wheel color & % text
      const overall = (typeof data.overall_score==='number') ? data.overall_score : 0;
      setScoreWheel(overall);

      // AI/Human chips + writer label
      ...
      document.getElementById('writerLabel').innerHTML='Writer: <b>'+label+'</b>';
