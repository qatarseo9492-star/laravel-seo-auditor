...
<!-- ===== Clean Replace: Remove old wheels/icons, add Burning Wheel + Animated Icons (robust) ===== -->
<style>
  /* Ensure all likely headers align items consistently */
  .category-title, .category-header, .checklist .category-title, .checklist .group-title,
  .checklist .section-title, .checklist .title, .checklist .header, .group .header,
  .card .header, .card-header, .cl-header {
    display:flex; align-items:center; gap:.65rem;
  }
  .cat-wheel{ margin-right:.45rem; }
  .cat-ico{ width:22px; height:22px; margin-right:.35rem; vertical-align:-3px; }
</style>
<script>
(function(){
  // --- Utilities ---
  ...
  const findHeaders = ()=>{
    let nodes = $all('.category-title, .category-header, .checklist .category-title, .checklist .group-title, .checklist .section-title, .checklist .title, .checklist .header, .group .header, .card .header, .card-header, .cl-header, h2, h3');
    // Keep only those that have checkboxes somewhere in their nearest card/section container
    nodes = nodes.filter(h => {
      const root = categoryRoot(h);
      return root && $all('input[type="checkbox"]', root).length >= 2;
    });
    // De-dup by element
    return Array.from(new Set(nodes));
  };
  const categoryRoot = (h)=> h.closest('.card, .panel, .category, .checklist-group, section, .group, .list, .block, article') || h.parentElement;

  // Remove any previous wheel/icon placeholders that might conflict
  function purgeOldHeaderStuff(h){
    // remove known wheel classes
    $all('.score-wheel, .radial-progress, .circular-progress, .donut, .progress-ring, svg.progress, .old-wheel, [data-old-wheel]', h).forEach(n=> n.remove());
    // hide icon-font <i> tags
    $all('i', h).forEach(i=> i.style.display = 'none');
    // heuristic: remove empty square placeholders (40-72px)
    const first = h.firstElementChild;
    if(first && !first.classList.contains('cat-wheel') && first.tagName.toLowerCase() !== 'svg'){
      const cs = getComputedStyle(first);
      const w = parseFloat(cs.width), hi = parseFloat(cs.height);
      if(!first.textContent.trim() && Math.abs(w-hi) < 2 && w>=40 && w<=72){ first.remove(); }
    }
  }

  // Install burning wheel and animated icon on a header
  function ensureWheel(h){
    let w = h.querySelector('.cat-wheel');
    if(!w){
      w = document.createElement('span');
      w.className = 'cat-wheel';
      w.innerHTML = '<span class="fire"></span><b class="val">0%</b>';
      h.insertBefore(w, h.firstChild);
    }
    return w;
  }

  // --- ICON sprite injection (once) ---
  function ensureSprite(){
    if(document.getElementById('aurora-ico-sprite')) return;
    const tpl = `
<svg id="aurora-ico-sprite" aria-hidden="true" focusable="false" style="position:absolute;width:0;height:0;overflow:hidden">
  <defs>
    <linearGradient id="grad-aurora-ico" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%"   stop-color="#7c5cff"/>
      <stop offset="50%"  stop-color="#22c3f7"/>
  ...
  <symbol id="ico-shield" viewBox="0 0 24 24"><path class="line" d="M12 3l7 3v5c0 5-3.5 8-7 10-3.5-2-7-5-7-10V6l7-3z" /><path class="line" d="M9.5 12l1.8 1.8 3.2-3.2" /></symbol>
  <symbol id="ico-link" viewBox="0 0 24 24"><path class="line" d="M10 14L8 16a4 4 0 0 1-6-6l2-2" /><path class="line" d="M14 10l2-2a4 4 0 0 1 6 6l-2 2" /><path class="line" d="M8 12h8" /></symbol>
  <symbol id="ico-media" viewBox="0 0 24 24"><rect class="line" x="3" y="5" width="18" height="14" rx="2" /><circle class="line" cx="9" cy="9" r="2" /><path class="line" d="M3 17l5-5 4 4 3-3 6 6" /></symbol>
  <symbol id="ico-structure" viewBox="0 0 24 24"><path class="line" d="M9 6h12M9 12h12M9 18h12" /><circle class="line" cx="5" cy="6" r="1.2" /><circle class="line" cx="5" cy="12" r="1.2" /><circle class="line" cx="5" cy="18" r="1.2" /></symbol>
  <symbol id="ico-mobile" viewBox="0 0 24 24"><rect class="line" x="7" y="3" width="10" height="18" rx="2"/><circle class="line" cx="12" cy="17" r="1"/></symbol>
  <symbol id="ico-speed" viewBox="0 0 24 24"><path class="line" d="M4 15a8 8 0 1 1 16 0" /><path class="line" d="M12 15l4-4" /><path class="line" d="M6 19h12" /></symbol>
  <symbol id="ico-ux" viewBox="0 0 24 24"><path class="line" d="M6 4l8 8-4 1 3 6 2-1 2 4" /></symbol>
  <symbol id="ico-cta" viewBox="0 0 24 24"><circle class="line" cx="12" cy="12" r="8" /><circle class="line" cx="12" cy="12" r="4" /><circle class="line" cx="12" cy="12" r="1" /></symbol>
  <symbol id="ico-schema" viewBox="0 0 24 24"><circle class="line" cx="5" cy="12" r="2"/><circle class="line" cx="12" cy="5" r="2"/><circle class="line" cx="19" cy="12" r="2"/><circle class="line" cx="12" cy="19" r="2"/><path class="line" d="M7 12h10M12 7v10M7 11L11 7M17 11l-4-4M7 13l4 4M17 13l-4 4"/></symbol>
  <symbol id="ico-breadcrumb" viewBox="0 0 24 24"><path class="line" d="M5 6l5 6-5 6M14 6l5 6-5 6"/></symbol>
  <symbol id="ico-url" viewBox="0 0 24 24"><path class="line" d="M7 12a5 5 0 0 1 5-5h1" /><path class="line" d="M17 12a5 5 0 0 1-5 5h-1" /><path class="line" d="M8 12h8" /></symbol>
  <symbol id="ico-entity" viewBox="0 0 24 24"><circle class="line" cx="12" cy="12" r="2"/><ellipse class="line" cx="12" cy="12" rx="8" ry="4"/><ellipse class="line" cx="12" cy="12" rx="4" ry="8" transform="rotate(60 12 12)"/><ellipse class="line" cx="12" cy="12" rx="4" ry="8" transform="rotate(-60 12 12)"/></symbol>
  <symbol id="ico-star" viewBox="0 0 24 24"><path class="line" d="M12 3l2.8 5.7L21 10l-4.5 4.2L17.6 21 12 17.8 6.4 21l1.1-6.8L3 10l6.2-1.3L12 3z"/></symbol>
</svg>`;
    document.body.insertAdjacentHTML('afterbegin', tpl);
  }

  // Style for animated lines on icons (once)
  function ensureIconCSS(){
    if(document.getElementById('aurora-ico-css')) return;
    const css = document.createElement('style');
    css.id = 'aurora-ico-css';
    css.textContent = `
      .cat-ico{ filter: drop-shadow(0 6px 16px rgba(124,92,255,.22)); animation: catHue 12s linear infinite; }
      @keyframes catHue { to { filter: hue-rotate(360deg) drop-shadow(0 6px 16px rgba(124,92,255,.22)); } }
      .cat-ico .line{ fill:none; stroke:url(#grad-aurora-ico); stroke-width:2; stroke-linecap:round; stroke-linejoin:round; stroke-dasharray:120; stroke-dashoffset:360; animation: catDash 14s linear infinite; }
      .cat-ico .fill{ fill:url(#grad-aurora-ico); }
      @keyframes catDash { to { stroke-dashoffset:0; } }
    `;
  ...
  const ICONS = [
    {k:['content','intent','readability','text','quality'], id:'ico-doc'},
    {k:['keyword','semantic','terms'], id:'ico-key'},
    {k:['meta','description','title','tag'], id:'ico-tag'},
    {k:['canonical','duplicate'], id:'ico-canon'},
    {k:['sitemap','index'], id:'ico-sitemap'},
    ...
    {k:['link','internal','external','backlink'], id:'ico-link'},
    {k:['image','media','video'], id:'ico-media'},
    {k:['structure','heading','h1','h2','outline','logic'], id:'ico-structure'},
    {k:['mobile','responsive'], id:'ico-mobile'},
    {k:['speed','performance','vitals','lcp','cls','inp'], id:'ico-speed'},
    {k:['ux','usability','accessibility','experience'], id:'ico-ux'},
    {k:['cta','conversion','button'], id:'ico-cta'},
    {k:['schema','json-ld','rich result'], id:'ico-schema'},
    {k:['breadcrumb'], id:'ico-breadcrumb'},
    {k:['url','slug'], id:'ico-url'},
    {k:['entity','entities','topic','context'], id:'ico-entity'},
  ];
  const pickIcon = (label)=>{
    const t = (label||'').toLowerCase();
    for(const m of ICONS){
  ...
  function ensureIcon(h){
    if(h.querySelector('svg.cat-ico')) return;
    const lbl = (h.textContent || '').trim();
    const sym = pickIcon(lbl);
    const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    svg.setAttribute('class','cat-ico');
    svg.setAttribute('viewBox','0 0 24 24');
    svg.setAttribute('aria-hidden','true');
    ...
    use.setAttribute('href','#'+sym);
    svg.appendChild(use);
    // Place AFTER wheel (2nd position)
    if(h.firstChild && h.firstChild.classList && h.firstChild.classList.contains('cat-wheel')){
      h.insertBefore(svg, h.childNodes[1] || h.firstChild.nextSibling);
    } else {
      h.insertBefore(svg, h.firstChild);
    }
  }

  function computePctForHeader(h){
    const root = categoryRoot(h);
  ...
  function updateWheelOnHeader(h){
    const w = ensureWheel(h);
    const pct = computePctForHeader(h);
    w.style.setProperty('--pct', pct);
    const val = w.querySelector('.val'); if(val) val.textContent = pct + '%';
  }
  ...
  function init(){
    ...
    headers.forEach(h=>{
      purgeOldHeaderStuff(h);
      ensureWheel(h);
      ensureIcon(h);
      updateWheelOnHeader(h);
    });
    // Listen for checkbox changes anywhere
    document.addEventListener('change', (e)=>{
      if(e.target && e.target.matches('input[type="checkbox"]')){
        const h = (e.target.closest('.card, .panel, .category, .checklist-group, section, .group, .list, .block, article')||document).querySelector('.category-title, .category-header, .group-title, .section-title, .title, .header, h2, h3');
        if(h) updateWheelOnHeader(h);
      }
    });
    // Safety net refresh
    setInterval(()=> headers.forEach(updateWheelOnHeader), 4000);
  }
