...
@push('styles')
<style>
/* Header / chips / buttons / analyzer shell / checklist styles (trimmed from earlier to keep single source of truth) */
header.site{
  display:flex;align-items:center;justify-content:space-between;
  padding:16px 16px 20px;border-bottom:1px solid var(--line);
  background:linear-gradient(180deg,rgba(255,255,255,.06),rgba(255,255,255,.02));
  backdrop-filter:saturate(140%) blur(10px); border-radius:22px;
}
.brand{display:flex;align-items:center;gap:14px}
.brand-badge{
  width:64px;height:64px;border-radius:16px;display:grid;place-items:center;
  background:radial-gradient(120px 90px at 20% 15%, #2f1b6f55 0%, transparent 60%),linear-gradient(135deg, rgba(141,105,255,.22), rgba(54,230,255,.22));
  border:1px solid rgba(255,255,255,.12); color:#ffdce5; font-size:1.2rem;
  box-shadow:0 6px 30px rgba(54,230,255,.18), inset 0 0 18px rgba(141,105,255,.16);
}
.hero-title{
  font-size:clamp(1.6rem,4.2vw,3.2rem); font-weight:1000; line-height:1.05; margin:0;
  letter-spacing:.5px; background:linear-gradient(90deg,#b892ff,#36e6ff 45%,#ff7b6b 90%);
  -webkit-background-clip:text; -webkit-text-fill-color:transparent; text-shadow:0 0 28px rgba(155,92,255,.22);
}
.btn{--pad:.75rem 1.05rem;display:inline-flex;align-items:center;gap:.55rem;padding:var(--pad);border-radius:14px;border:1px solid transparent;cursor:pointer;font-weight:900;letter-spacing:.18px;transition:.22s}
.btn-neon{background:linear-gradient(135deg,#36e6ff,#8d69ff);box-shadow:0 8px 30px rgba(61,226,255,.25);color:#001018}
.btn-ghost{background:rgba(255,255,255,.05);border-color:rgba(255,255,255,.16);color:#fff}
.btn-danger{background:linear-gradient(135deg,#ff3b5c,#ff8a5b);color:#fff;box-shadow:0 8px 30px rgba(255,59,92,.25)}
.analyzer{margin-top:20px;background:linear-gradient(180deg,rgba(255,255,255,.06),rgba(255,255,255,.03));border:1px solid rgba(255,255,255,.1);border-radius:22px;box-shadow:var(--shadow);padding:22px}
...
.analyze-form input[type="url"]{width:100%; padding:1rem 1.2rem; border-radius:14px; border:1px solid #1b1b35; background:#0b0d21; color:var(--text)}
...
@media (max-width:992px){.category-card{grid-column:span 12}}
</style>
@endpush
...
@section('content')
  <header class="site">
    <div class="brand">
      ...
      <h1 class="hero-title">Semantic SEO Master Analyzer</h1>
    </div>
  ...
  <section class="analyzer" id="analyzer">
    ...
    <div class="score-area" style="display:grid;grid-template-columns:260px 1fr;gap:1.1rem;align-items:center;margin-top:.6rem">
    ...
    @php $labels = [
      1=>'Define search intent & primary topic', 2=>'Map target & related keywords (synonyms/PAA)',
      3=>'H1 includes primary topic naturally', 4=>'Integrate FAQs / questions with answers',
      5=>'Readable, NLP‑friendly language', 6=>'Title tag (≈50–60 chars) w/ primary keyword',
      7=>'Meta description (≈140–160 chars) + CTA', 8=>'Canonical tag set correctly',
      9=>'Indexable & listed in XML sitemap', 10=>'E‑E‑A‑T signals (author, date, expertise)',
      11=>'Unique value vs. top competitors', 12=>'Facts & citations up to date',
      13=>'Helpful media (images/video) w/ captions', 14=>'Logical H2/H3 headings & topic clusters',
      15=>'Internal links to hub/related pages', 16=>'Clean, descriptive URL slug',
      17=>'Breadcrumbs enabled (+ schema)', 18=>'Mobile‑friendly, responsive layout',
      19=>'Optimized speed (compression, lazy‑load)', 20=>'Core Web Vitals passing (LCP/INP/CLS)',
      21=>'Clear CTAs and next steps', 22=>'Primary entity clearly defined',
      23=>'Related entities covered with context', 24=>'Valid schema markup (Article/FAQ/Product)',
    ...
    <div class="analyzer-grid">
      @foreach ([
        ...
        <article class="category-card">
          <header class="category-head" style="display:grid;grid-template-columns:auto 1fr auto;gap:.75rem;align-items:center">
            <span class="category-icon" style="width:46px;height:46px;border-radius:12px;display:inline-flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#36e6ff33,#8d69ff33);color:#fff;font-size:1.1rem;border:1px solid rgba(255,255,255,.18)">
              <i class="fas {{ $c[3] }}"></i>
            </span>
            <div>
              <h3 class="category-title" style="margin:0;font-size:1.08rem;background:linear-gradient(90deg,#36e6ff,#8d69ff,#ff3b5c);-webkit-background-clip:text;-webkit-text-fill-color:transparent;font-weight:900">{{ $c[0] }}</h3>
              <p class="category-sub" style="margin:.15rem 0 0;color:var(--text-dim)">—</p>
            </div>
  ...
  <footer class="site">
    <div class="footer-brand"><span class="dot" style="width:8px;height:8px;border-radius:50%;background:linear-gradient(135deg,#36e6ff,#8d69ff)"></span><strong>Semantic SEO Master</strong></div>
  ...
  <div class="modal" id="tipModal" role="dialog" aria-modal="true" aria-labelledby="modalTitle" style="display:none;align-items:center;justify-content:center">
    <div class="modal-card" style="width:min(1000px,96vw);background:var(--panel-2);border:1px solid rgba(255,255,255,.12);border-radius:16px;box-shadow:var(--shadow);padding:16px">
      <div class="modal-header" style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.6rem">
        <h3 class="modal-title" id="modalTitle" style="margin:0;font-size:1.2rem">Improve</h3>
        <button class="modal-close" id="modalClose" style="background:transparent;border:1px solid rgba(255,255,255,.2);border-radius:10px;color:#fff;padding:.35rem .6rem;cursor:pointer"><i class="fa-solid fa-xmark"></i></button>
      </div>
      <div class="tabs" style="display:flex;gap:.4rem;margin:.4rem 0;flex-wrap:wrap">
        <button class="tab active" data-tab="tipsTab" style="padding:.35rem .7rem;border-radius:10px;border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.06);cursor:pointer;font-weight:800">Tips</button>
        <button class="tab" data-tab="examplesTab" style="padding:.35rem .7rem;border-radius:10px;border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.06);cursor:pointer;font-weight:800">Examples</button>
        <button class="tab" data-tab="humanTab" style="padding:.35rem .7rem;border-radius:10px;border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.06);cursor:pointer;font-weight:800">Human‑like</button>
        <button class="tab" data-tab="aiTab" style="padding:.35rem .7rem;border-radius:10px;border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.06);cursor:pointer;font-weight:800">AI‑like</button>
        <button class="tab" data-tab="fullTab" style="padding:.35rem .7rem;border-radius:10px;border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.06);cursor:pointer;font-weight:800">Full Text</button>
      </div>
      <div class="tabpanes">
        <div id="tipsTab" class="active"><ul id="modalList"></ul></div>
        <div id="examplesTab"><div class="pre" id="examplesPre" style="white-space:pre-wrap;background:#0b0d21;border:1px solid #1b1b35;border-radius:12px;padding:12px;color:#cfd3f6;max-height:60vh;overflow:auto">—</div></div>
        <div id="humanTab"><div class="pre" id="humanSnippetsPre" style="white-space:pre-wrap;background:#0b0d21;border:1px solid #1b1b35;border-radius:12px;padding:12px;color:#cfd3f6;max-height:60vh;overflow:auto">Run Analyze to view human‑like snippets.</div></div>
        <div id="aiTab"><div class="pre" id="aiSnippetsPre" style="white-space:pre-wrap;background:#0b0d21;border:1px solid #1b1b35;border-radius:12px;padding:12px;color:#cfd3f6;max-height:60vh;overflow:auto">Run Analyze to view AI‑like snippets.</div></div>
        <div id="fullTab"><div class="pre" id="fullTextPre" style="white-space:pre-wrap;background:#0b0d21;border:1px solid #1b1b35;border-radius:12px;padding:12px;color:#cfd3f6;max-height:60vh;overflow:auto">Run Analyze to load full text.</div></div>
      </div>
    </div>
  </div>
@endsection
...
(function(){
  ...
  layer('linesCanvas',140,130,a=>`rgba(54,230,255,${a})`,1.1);
  layer('linesCanvas2',110,120,a=>`rgba(255,59,92,${a*0.6})`,0.9);
})();
...
(function(){
  try {
    ...
    void main(){
      ...
      vec3 base=mix(vec3(.24,.88,1.),vec3(1.,.38,.45),uv.x);
      vec3 c=mix(base, vec3(1.,.82,.38), 0.35*uv.y);
      o=vec4(c*d, .72*d);
    }`;
  ...
  } catch(err){ console.warn('SmokeFX disabled:', err); }
})();

// Checklist + progress + row accent helpers
(function(){
...
(function(){
  ...
  async function analyze(){
    ...
    try{
      ...
      document.dispatchEvent(new Event('change')); // progress

      // Wheel + badges
      const overall = (typeof data.overall_score==='number') ? data.overall_score : 0;
      setScoreWheel('overall', overall);
...
// Ensure API exists before any manual calls
window.addEventListener('DOMContentLoaded', function(){
  if (typeof window.setScoreWheel === 'function' &&
      document.querySelector('[data-wheel-id="overall"]')) {
