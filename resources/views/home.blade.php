{{-- resources/views/home.blade.php — v2025-08-24ab-fixed --}}
<!DOCTYPE html>
...
@media print{#linesCanvas,#smokeCanvas,#backTop{display:none!important}}
</style>
</head>
<body>
  ...
  <main class="analyzer" id="analyzer" role="main">
    ...
    <div class="analyze-box" style="margin-top:12px;background:rgba(255,255,255,.02);border:1px solid rgba(255,255,255,.08);border-radius:16px;padding:14px">
      <form id="analyzeForm" onsubmit="event.preventDefault(); analyze(); return false;">
        ...
        <!-- Delegation fallback (handles CSP/defers) -->
        <script data-cfasync="false">
          document.addEventListener('click', function(e){
            var b = e.target.closest('#analyzeBtn');
            if(!b) return;
            e.preventDefault();
            (function wait(){ 
              if (typeof window.analyze==='function') return window.analyze();
              setTimeout(wait,80);
            })();
          }, true);
        </script>
    ...
    <section id="readPanel" class="panel">
      ...
      <div style="margin-top:.5rem;color:var(--text-dim)">Aim for clear sentences, varied vocabulary, and limited repetition. Target grade ~7.</div>
    </section>
...
(function(){
  ...
  async function analyze(){
    ...
    try{
      ...
      if(!url){
        statusEl && (statusEl.textContent = 'Please enter a valid URL (include http/https).');
        input && input.focus();
        return;
      }
  ...
  document.addEventListener('DOMContentLoaded', function(){
    try{
      ...
      document.getElementById('resetChecklist')?.addEventListener('click', function(){
        ...
        ['contentScoreInline','humanPct','aiPct','hvaiHumanPct','hvaiAIPct'].forEach(id=>{ var el=document.getElementById(id); if(el) el.textContent='—'; });
        var badge=document.getElementById('aiBadge'); if(badge && badge.querySelector('b')) badge.querySelector('b').textContent='—';
        ...
        document.getElementById('hvaiHuman').style.width='50%'; document.getElementById('hvaiAI').style.width='50%';
        window.Water?.reset();
      });
