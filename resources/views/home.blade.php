...
<style>
/* ——— the long CSS is unchanged from the working version you confirmed ——— */
/* (keeping everything to preserve your effects; omitted comments for brevity) */
...
#linesCanvas{opacity:.35}#brainCanvas{opacity:.28}
...
.btn-analyze i{color:#03291a}.btn-print i{color:#051735}.btn-reset i{color:#2a1403}.btn-export i{color:#2a0730}
...
.score-container{width:220px}.score-gauge{position:relative;width:100%;aspect-ratio:1/1}.gauge-svg{width:100%;height:auto;display:block}
.score-mask-rect{transition:all .6s cubic-bezier(.22,1,.36,1)}.score-wave1{animation:scoreWave 8s linear infinite}.score-wave2{animation:scoreWave 11s linear infinite reverse}
@keyframes scoreWave{from{transform:translateX(0)}to{transform:translateX(-210px)}}
.score-text{font-size:clamp(2.2rem,4.2vw,3.1rem);font-weight:1000;fill:#fff;text-shadow:0 0 18px rgba(255,32,69,.25)}.multiHueFast{filter:hue-rotate(var(--hue)) saturate(140%);will-change:filter}
...
.water-svg{position:absolute;inset:0;width:100%;height:100%;z-index:1}.water-mask-rect{transition:all .25s ease-out}
...
.category-card>*{position:relative;z-index:1}@keyframes borderGlow{0%{filter:hue-rotate(0)}100%{filter:hue-rotate(360deg)}}
...
@keyframes rowFlash{0%{background:linear-gradient(90deg,rgba(61,226,255,.20),rgba(155,92,255,.16))}100%{background:linear-gradient(180deg,rgba(255,255,255,.04),rgba(255,255,255,.02)),radial-gradient(100% 120% at 0% 0%,rgba(61,226,255,.06),transparent 30%),radial-gradient(120% 100% at 100% 0%,rgba(155,92,255,.05),transparent 35%)}}
...
.footer-brand{display:flex;align-items:center;gap:.6rem}.footer-brand .dot{width:8px;height:8px;border-radius:50%;background:linear-gradient(135deg,#3de2ff,#9b5cff)}
.footer-links a{color:var(--text-dim);margin-left:.9rem}.footer-links a:hover{color:#fff;text-decoration:underline}
...
.lang-card{width:240px;max-height:70vh;overflow:auto;padding:.6rem;background:rgba(10,12,28,.65);border:1px solid rgba(255,255,255,.14);border-radius:14px;box-shadow:0 18px 60px rgba(0,0,0,.5);backdrop-filter:blur(10px)}
...
@media (max-width:992px){.category-card{grid-column:span 12}.score-container{width:190px}footer.site{flex-direction:column;align-items:flex-start}.analyze-row{grid-template-columns:1fr auto auto}}
@media (max-width:768px){.wrap{padding:18px 4%}header.site{flex-direction:column;align-items:flex-start;gap:.6rem}.score-area{flex-direction:column;align-items:flex-start;gap:.8rem}.score-container{width:170px}.analyze-row{grid-template-columns:1fr}.analyze-row .btn{width:100%;justify-content:center}.share-dock{top:auto;bottom:10px;right:50%;transform:translateX(50%);flex-direction:row;padding:.35rem .45rem;border-radius:999px;gap:.4rem;background:rgba(10,12,28,.55)}.share-btn{width:44px;height:44px;border-radius:999px}.share-native{display:grid}.checklist-item{grid-template-columns:1fr auto auto}.checklist-item .improve-btn{grid-column:1/-1;justify-self:flex-start;margin-top:.25rem}}
...
const I18N = {/* … same dictionary as before … */};
const LANGS = [/* … same 10 languages as before … */];
(function(){ /* … same language init code … */ })();
...
/* Smoky water overlays (unchanged) */
const CatSmoke = (function(){ /* … same as previous … */ })();
const CompSmoke = (function(){ /* … same as previous … */ })();

/* Water progress controller (+ smoke) */
const WaterSmoke = (function(){ /* … same as previous … */ })();
const Water = (function(){ /* … same as previous … */ })();

/* ------------------------ Share links + back to top ------------------------ */
(function(){ /* … same share links … */ })();
(function(){ /* … same back-to-top … */ })();
...
function updateCategoryBars(){ /* … same as previous … */ }
function autoTickByScores(scoresMap){ /* … same as previous … */ }
...
async function analyze(){
  ...
  // 1) GET named route /analyze-json
  try{
    const qs = new URLSearchParams({ url }).toString();
    const res = await fetch(`{{ route('analyze.json', [], false) }}?${qs}`, {
      method:'GET',
  ...
  // 2) POST named route /analyze (CSRF)
  if (!ok){
    try{
      const res = await fetch(`{{ route('analyze', [], false) }}`, {
        method:'POST',
        headers:{
  ...
  // 3) GET named route /analyze fallback
  if (!ok){
    try{
      const qs = new URLSearchParams({ url }).toString();
      const res = await fetch(`{{ route('analyze', [], false) }}?${qs}`, {
        method:'GET',
        headers:{ 'Accept':'application/json', 'X-Requested-With':'XMLHttpRequest' }
      });
      status = res.status; text = await res.text();
...
/* --------- Cloudy background rendering (unchanged) --------- */
(function(){ /* lines layer */ const c=document.getElementById('linesCanvas'); if(!c)return; const ctx=c.getContext('2d'); let dpr=Math.min(2,devicePixelRatio||1); function resize(){c.width=Math.floor(innerWidth*dpr);c.height=Math.floor(innerHeight*dpr);ctx.setTransform(dpr,0,0,dpr,0,0)} function draw(t){ctx.clearRect(0,0,innerWidth,innerHeight);const w=innerWidth,h=innerHeight,rows=12,spacing=Math.max(60,h/rows);for(let i=-2;i<rows+2;i++){const y=i*spacing+(t*0.02%spacing);const g=ctx.createLinearGradient(0,y,w,y+80);g.addColorStop(0,'rgba(61,226,255,0.03)');g.addColorStop(0.5,'rgba(155,92,255,0.05)');g.addColorStop(1,'rgba(255,32,69,0.03)');ctx.strokeStyle=g;ctx.lineWidth=1.2;ctx.beginPath();ctx.moveTo(-100,y);ctx.lineTo(w+100,y+80);ctx.stroke()}requestAnimationFrame(draw)} addEventListener('resize',resize,{passive:true});resize();requestAnimationFrame(draw);})();
(function(){ /* fog layer */ const c=document.getElementById('brainCanvas'); if(!c)return; const ctx=c.getContext('2d'); let dpr=Math.min(2,devicePixelRatio||1); let blobs=[]; function resize(){c.width=Math.floor(innerWidth*dpr);c.height=Math.floor(innerHeight*dpr);ctx.setTransform(dpr,0,0,dpr,0,0);blobs=Array.from({length:36}).map(()=>({x:Math.random()*innerWidth,y:Math.random()*innerHeight,r:40+Math.random()*120,vx:(Math.random()-.5)*.2,vy:(Math.random()-.5)*.2,hue:180+Math.random()*140,a:.08+.08*Math.random()}))} function draw(){ctx.clearRect(0,0,innerWidth,innerHeight);ctx.globalCompositeOperation='lighter';for(const b of blobs){b.x+=b.vx;b.y+=b.vy;if(b.x<-200)b.x=innerWidth+200;if(b.x>innerWidth+200)b.x=-200;if(b.y<-200)b.y=innerHeight+200;if(b.y>innerHeight+200)b.y=-200;const g=ctx.createRadialGradient(b.x,b.y,0,b.x,b.y,b.r);g.addColorStop(0,`hsla(${b.hue},85%,65%,${b.a})`);g.addColorStop(1,`hsla(${(b.hue+60)%360},85%,55%,0)`);ctx.fillStyle=g;ctx.beginPath();ctx.arc(b.x,b.y,b.r,0,Math.PI*2);ctx.fill()}requestAnimationFrame(draw)} addEventListener('resize',resize,{passive:true});resize();requestAnimationFrame(draw);})();
</script>
</body>
</html>
