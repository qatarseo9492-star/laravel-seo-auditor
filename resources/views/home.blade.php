...
/* HVAI tech-lines background (mouse reactive) */
.hvai{position:relative; overflow:hidden; isolation:isolate;}
.hvai .hvai-bg-canvas{position:absolute; inset:0; z-index:0; pointer-events:none; opacity:.9; mix-blend-mode:screen}
.hvai > *:not(.hvai-bg-canvas){position:relative; z-index:1}
/* colorful icon pills for HVAI */
.hvai-head .ico{ 
  width:40px; height:40px; border-radius:12px; display:inline-grid; place-items:center; color:#fff;
  background:conic-gradient(from 0deg, #3de2ff, #22c55e, #9b5cff, #f59e0b, #ff2045, #3de2ff);
  box-shadow:0 8px 20px rgba(0,0,0,.28), inset 0 0 0 1px rgba(255,255,255,.14);
}
.hvai-chip i{color:#fff}


/* HVAI label cosmetics */
.hvai .hvai-label span{font-weight:900; letter-spacing:.2px}
.hvai .hvai-label i{margin-right:.35rem; filter:drop-shadow(0 2px 8px rgba(61,226,255,.25))}
.hvai .hvai-track{box-shadow:inset 0 0 0 1px rgba(255,255,255,.04), 0 6px 18px rgba(0,0,0,.25)}
.hvai .hvai-fill.human{background:linear-gradient(90deg,#22c55e,#3de2ff,#9b5cff)}
.hvai .hvai-fill.ai{background:linear-gradient(270deg,#ef4444,#f59e0b,#ff2045)}
...
<div class="wrap">
  ...
  <main class="analyzer" id="analyzer" role="main">
    ...
    <section id="detectorPanel" class="hvai" style="display:none">
      <canvas id="hvaiLines" class="hvai-bg-canvas" aria-hidden="true"></canvas>
...
<script>
(function(){
  function hvaiTechLines(){
    var sect = document.getElementById('detectorPanel');
    var c = document.getElementById('hvaiLines');
    if(!sect || !c) return;
    var ctx = c.getContext('2d');
    var dpr = Math.min(2, window.devicePixelRatio || 1);
    var W=0,H=0, mouseX=0.5, mouseY=0.5, t0=performance.now();

    function resize(){
      var bb = sect.getBoundingClientRect();
    ...
    function onMouse(e){
      ...
      mouseX = Math.max(0, Math.min(1, (e.clientX - r.left) / Math.max(1,r.width)));
      mouseY = Math.max(0, Math.min(1, (e.clientY - r.top)  / Math.max(1,r.height)));
    }
    sect.addEventListener('mousemove', onMouse, {passive:true});
    ...
    function draw(now){
      ...
      for (var i=-2; i<rows+2; i++){
        var y = i*spacing + (t*20 % spacing);
        var grad = ctx.createLinearGradient(0,y, W, y+80);
        grad.addColorStop(0, 'rgba(61,226,255,0.10)');
        grad.addColorStop(0.5, 'rgba(155,92,255,0.12)');
        grad.addColorStop(1, 'rgba(255,32,69,0.10)');
        ctx.strokeStyle = grad;
        ctx.lineWidth = 1.2;
        ctx.beginPath();
        ctx.moveTo(-80, y);
        ctx.lineTo(W+80, y + 80);
        ctx.stroke();
      }
      // Reactive "tech" beams
      var beams = 7;
      for (var b=0; b<beams; b++){
        var phase = (t*0.8 + b*0.2);
        var x = (mouseX*W*0.6) + Math.sin(phase)*W*0.25 + (b*W/beams)*0.2;
        var y = (mouseY*H*0.6) + Math.cos(phase*1.2)*H*0.25;
        var g2 = ctx.createLinearGradient(x-120, y-60, x+220, y+120);
        g2.addColorStop(0, 'rgba(61,226,255,0.18)');
        g2.addColorStop(0.5, 'rgba(155,92,255,0.20)');
        g2.addColorStop(1, 'rgba(255,32,69,0.18)');
        ctx.strokeStyle = g2;
        ctx.lineWidth = 1.6;
        ctx.beginPath();
        ctx.moveTo(x-200, y-100);
        ctx.lineTo(x+W+200, y+H+100);
        ctx.stroke();
      }
      requestAnimationFrame(draw);
    }
  ...
  if (document.readyState === 'loading'){
    document.addEventListener('DOMContentLoaded', hvaiTechLines);
  } else {
    hvaiTechLines();
  }
})();
