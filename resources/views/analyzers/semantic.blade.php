...
<style>
  ...
  .king{width:44px;height:44px;border-radius:12px;display:grid;place-items:center;background:#101010;border:1px solid #ffffff24}
  ...
  .url-row{display:flex;align-items:center;gap:10px;border:1px solid #ffffff24;background:#0b0b0b;border-radius:12px;padding:8px 10px}
  ...
  /* Water bar */
  .waterbox{position:relative;height:16px;border-radius:9999px;overflow:hidden;border:1px solid #ffffff22;background:#0b0b0b}
...
<section class="maxw px-4 pb-10">
  ...
  <div style="display:grid;grid-template-columns:230px 1fr;gap:16px;align-items:center;margin-top:10px">
    <div style="display:grid;place-items:center;border-radius:16px;padding:8px;background:#05050a;border:1px solid #ffffff12">
  ...
  {{-- Analyze toolbar (directly under wheel) --}}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function callAnalyzer(url){
    ...
    try{
      ...
      if([404,405,419].includes(res.status)){
        // fallback to web POST (named route in web.php)
  ...
  const KB = {
    // ----- Content & Keywords
    'Define search intent & primary topic': {why:'Matching intent is the #1 driver of relevance and dwell time.', tips:['State the problem and outcome in the intro.','Align format with intent (guide, checklist, comparison, etc.).','Use examples that match the reader’s goal.'], link:'https://ahrefs.com/blog/search-intent/'},
    'Map target & related keywords (synonyms/PAA)': {why:'Covering variants improves recall and topical completeness.', tips:['List 6–12 key variants and 5–10 PAA questions.','Answer PAA in short, direct paragraphs.','Use variants naturally in H2/H3 and body.'], link:'https://developers.google.com/search/docs/fundamentals/seo-starter-guide'},
    'H1 includes primary topic naturally': {why:'Clear page topic helps users and algorithms.', tips:['One H1 per page.','Put the primary topic near the start.','Avoid clickbait; be descriptive.'], link:'https://web.dev/learn/html/semantics/#headings'},
    'Integrate FAQs / questions with answers': {why:'Captures long-tail and can earn rich results.', tips:['Pick 3–6 common questions.','Answer each in 40–60 words.','Add FAQPage JSON-LD.'], link:'https://developers.google.com/search/docs/appearance/structured-data/faqpage'},
    'Readable, NLP-friendly language': {why:'Plain, direct writing improves comprehension and model parsing.', tips:['Short sentences (≤20 words).','Active voice, concrete nouns/verbs.','Define jargon on first use.'], link:'https://www.plainlanguage.gov/guidelines/'},

    // ----- Content Quality
    'E-E-A-T signals (author, date, expertise)': {why:'Trust signals reduce bounce and build credibility.', tips:['Add author bio + credentials.','Show last updated date.','Link to About/Editorial policy.'], link:'https://developers.google.com/search/blog/2022/08/helpful-content-update'},
    'Unique value vs. top competitors': {why:'Differentiation is necessary to rank and retain.', tips:['Add original data, screenshots, or step-by-step.','Include pros/cons and decision criteria.','Summarize why your approach is better.'], link:'https://backlinko.com/seo-techniques'},
    'Facts & citations up to date': {why:'Freshness + accuracy boosts trust and SERP stability.', tips:['Cite primary sources.','Update stats ≤12 months old.','Use canonical/doi links where possible.'], link:'https://scholar.google.com/'},
    'Helpful media (images/video) w/ captions': {why:'Media breaks walls of text and improves clarity.', tips:['Add 3–6 illustrative figures.','Write descriptive captions.','Compress and lazy-load images.'], link:'https://web.dev/optimize-lcp/'},

    // ----- Structure & Architecture
    'Logical H2/H3 headings & topic clusters': {why:'Hierarchy helps both skimming and indexing.', tips:['Group related subtopics under H2.','Use H3 for steps/examples.','Keep sections 150–300 words.'], link:'https://moz.com/learn/seo/site-structure'},
    'Internal links to hub/related pages': {why:'Internal links distribute authority & context.', tips:['Link to 3–5 relevant hubs.','Use descriptive anchor text.','Add “Further reading” section.'], link:'https://ahrefs.com/blog/internal-links/'},
    'Clean, descriptive URL slug': {why:'Readable slugs improve CTR and clarity.', tips:['3–5 meaningful words.','Hyphens, lowercase.','Avoid dates if not needed.'], link:'https://developers.google.com/search/docs/crawling-indexing/url-structure'},
    'Breadcrumbs enabled (+ schema)': {why:'Breadcrumbs clarify location and can show in SERP.', tips:['Render visible breadcrumbs.','Add BreadcrumbList JSON-LD.','Keep depth logical.'], link:'https://developers.google.com/search/docs/appearance/structured-data/breadcrumb'},

    // ----- Technical Elements
    'Title tag (≈50–60 chars) w/ primary keyword': {why:'Title is still the strongest on-page signal.', tips:['Keep 50–60 chars.','Put the primary topic first.','Avoid truncation and duplication.'], link:'https://moz.com/learn/seo/title-tag'},
    'Meta description (≈140–160 chars) + CTA': {why:'Meta drives CTR which correlates with rankings.', tips:['140–160 chars.','Include benefit + soft CTA.','Match search intent.'], link:'https://moz.com/learn/seo/meta-description'},
    'Canonical tag set correctly': {why:'Avoid duplicates and consolidate signals.', tips:['One canonical per page.','Canonical to the preferred URL.','Check for conflicting canonicals.'], link:'https://developers.google.com/search/docs/crawling-indexing/consolidate-duplicate-urls'},
    'Indexable & listed in XML sitemap': {why:'Indexation is prerequisite to ranking.', tips:['Ensure not blocked by robots.','Include in XML sitemap.','Submit in Search Console.'], link:'https://developers.google.com/search/docs/crawling-indexing/overview'},

    // ----- User Signals & Experience
    'Mobile-friendly, responsive layout': {why:'Most traffic is mobile; poor UX kills engagement.', tips:['Use responsive breakpoints.','Tap targets ≥44px.','Avoid horizontal scroll.'], link:'https://search.google.com/test/mobile-friendly'},
    'Optimized speed (compression, lazy-load)': {why:'Speed affects abandonment and CWV metrics.', tips:['Compress (WebP/AVIF).','HTTP/2 + caching.','Lazy-load below-the-fold media.'], link:'https://web.dev/fast/'},
    'Core Web Vitals passing (LCP/INP/CLS)': {why:'Passing CWV improves experience and stability.', tips:['Preload hero image.','Minimize JS long tasks.','Reserve image/video space.'], link:'https://web.dev/vitals/'},
    'Clear CTAs and next steps': {why:'Clarity increases conversions and user task completion.', tips:['One primary CTA per view.','Action verbs.','Add “what happens next”.'], link:'https://www.nngroup.com/articles/call-to-action-buttons/'},

    // ----- Entities & Context
    'sameAs/Organization details present': {why:'Entity grounding disambiguates your brand.', tips:['Add Organization JSON-LD.','Include sameAs links.','Ensure NAP consistency.'], link:'https://schema.org/Organization'},
  ...
  /* Default categories (used if API returns none) */
  const DEFAULT_CATS = [
  ...
  analyzeBtn?.addEventListener('click', async e=>{
    ...
    try{
      ...
      /* Writer/Human/AI (heuristic, compact) */
      ...
      /* Status chips coloring (HTTP, Title, Meta) */
      function setBandChip(wrapperEl, value, score){
        if(!wrapperEl) return;
        wrapperEl.classList.remove('good','warn','bad');
        wrapperEl.classList.add(bandName(score));
      }
      // HTTP (assume 200 on success)
      chipHttp.textContent='200'; setBandChip(chipHttpWrap,'200', 90);
      // Title length band
      const tl = title.length; chipTitle.textContent = tl ? (tl+' chars') : '—';
      setBandChip(chipTitleWrap, tl, (tl>=50&&tl<=60)?85:(tl?65:40));
      // Meta length band
      const ml = meta.length; chipMeta.textContent = ml ? (ml+' chars') : '—';
      setBandChip(chipMetaWrap, ml, (ml>=140&&ml<=160)?85:(ml?65:40));
      // Others (we don’t have values client-side)
      ...
      /* Categories + checks (merge with defaults if API missing) */
      let cats = Array.isArray(data.categories) && data.categories.length ? data.categories : DEFAULT_CATS;

      // Merge: if API has some, ensure all default categories exist too (so nothing is “missing”)
      const namesInApi = new Set((data.categories||[]).map(c=>c.name));
      DEFAULT_CATS.forEach(def=>{
        if(!namesInApi.has(def.name)) cats.push(def);
      });

      // Enrich checks with KB and render
      catsEl.innerHTML='';
      cats.forEach(cat=>{
        ...
        ck.forEach(ch=>{
          ...
          row.innerHTML = `
            <div style="display:flex;align-items:center;gap:8px">
              <span class="w-3 h-3 rounded-full" style="display:inline-block;width:10px;height:10px;border-radius:9999px;background:${dot}"></span>
