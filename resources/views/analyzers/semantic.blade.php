...
@push('head')
<style>
  /* =============== Base page styles (New Stylish Redesign - V2) =============== */
  :root {
    --bg-dark-1: #1A1A1A;
    --bg-dark-2: #1F1F1F;
    --bg-dark-3: #262626;
...
(function(){
  const init = () => {
    ...
    function setWheel(elRing,elFill,elNum,container,score,prefix){const b=bandName(score);container.classList.remove('good','warn','bad');container.classList.add(b);if(elRing)elRing.style.setProperty('--v',score);if(elNum)elNum.innerHTML=`${score}<span>%</span>`;}
    ...
    analyzeBtn?.addEventListener('click', async e=>{
      ...
      try {
        ...
        if(mwRing) mwRing.style.setProperty('--v',score);
        if(mwNum) mwNum.innerHTML=`${score}<span>%</span>`;
        ...
        if(data.content_optimization) {
            ...
            if (co.nlp_score != null) {
                ...
                if(coNlpBadge) {
                    coNlpBadge.textContent = badgeText(co.nlp_score);
                    coNlpBadge.className = 'co-badge ' + badgeClass(co.nlp_score);
                }
                if(nlpTips) {
                    nlpTips.innerHTML = ''; 
        ...
        // Render SEO Ground Accordion
        renderAccordion(data, url);
        ...
        const lcpSeconds=pick(mobile.lcp_s,desktop.lcp_s,psi.lcp_s);
        const cls=pick(mobile.cls,desktop.cls,psi.cls);
        const inp=pick(mobile.inp_ms,desktop.inp_ms,psi.inp_ms);
        const ttfb=pick(mobile.ttfb_ms,desktop.ttfb_ms,psi.ttfb);
    ...
    function renderAccordion(data, url) {
        ...
        if (!seoGround) return;
        seoGround.innerHTML = ''; // Clear previous items
        ...
        CATS.forEach(cat => {
            ...
            cat.checks.forEach(lbl => {
                ...
                contentHTML += `
                    <div class="check" data-label="${lbl}" data-cat="${cat.name}">
                        <div style="display:flex;align-items:center;gap:8px">
                            <span style="display:inline-block;width:10px;height:10px;border-radius:9999px;background:${dot}"></span>
                            <div class="font-semibold" style="font-size:13px">${lbl}</div>
                        </div>
                        <div style="display:flex;align-items:center;gap:6px">
                            <span class="score-pill ${pill}">${s}</span>
                            <button class="improve-btn ${fill}" type="button">Improve</button>
                        </div>
                    </div>`;
            });
            ...
            item.innerHTML = `
                <div class="accordion-header">
                    <div class="accordion-title"><span class="icon">${cat.icon}</span> ${cat.name}</div>
        ...
        seoGround.addEventListener('click', function(e){
            ...
            if (header) {
                ...
                if (item.classList.contains('active')) {
                    ...
                    seoGround.querySelectorAll('.accordion-item').forEach(i => {
            ...
            if (improveBtn) {
                ...
                const label = checkEl.dataset.label;
                const catName = checkEl.dataset.cat;
                const score = checkEl.querySelector('.score-pill').textContent;
...
@section('content')
<section class="maxw px-4 pb-10" style="background-color: #1A1A1A;">
