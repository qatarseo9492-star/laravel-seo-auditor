...
<style>
  ...
  /* ================================================================= */
  /* === ✨ NEW STYLES for Readability & Tone Optimization ✨ === */
  /* ================================================================= */
  .rto-card {
    background: linear-gradient(145deg, #0c2b21, #0c1f2e);
    border: 1px solid var(--green-1);
    border-radius: 20px;
    padding: 16px;
    margin-top: 24px;
    box-shadow: 0 0 32px rgba(0, 255, 138, 0.3), inset 0 0 12px rgba(0, 0, 0, 0.5);
  }
  .rto-grid {
      display: grid;
   ...
   @media (max-width: 920px) { .rto-grid { grid-template-columns: 1fr; } }
  .rto-textarea {
      width: 100%;
      min-height: 200px;
      background: #111827;
  ...
  .rto-suggestion-item .original {
      color: #f87171;
      text-decoration: line-through;
      margin-right: 8px;
  }
  .rto-suggestion-item .suggestion {
      color: #6ee7b7;
  }
...
(function(){
  const init = () => {
    ...
    // --- ✨ NEW: Readability & Tone Optimization UI refs ✨ ---
    const rtoTextarea = $('#rtoTextarea');
    const rtoAnalyzeBtn = $('#rtoAnalyzeBtn');
    const rtoOverallTone = $('#rtoOverallTone');
    const rtoGradeLevel = $('#rtoGradeLevel');
    const rtoWordCount = $('#rtoWordCount');
    const rtoPassiveVoice = $('#rtoPassiveVoice');
    const rtoSuggestionsList = $('#rtoSuggestionsList');
    ...
    $('#analyzeBtn')?.addEventListener('click', async e=>{
      ...
      try{
        ...
        // ✨ NEW: Auto-populate and trigger Readability & Tone section ✨
        if (data.main_content) {
            rtoTextarea.value = data.main_content;
            rtoAnalyzeBtn.click(); // Programmatically click the button to run the analysis
        }
    ...
    aiBriefBtn?.addEventListener('click', async () => {
        ...
        if (!url) {
            aiBriefResult.textContent = 'Please enter a URL to analyze against.';
            return;
        }
    ...
    // --- Readability & Tone Manual Analysis ---
    rtoAnalyzeBtn?.addEventListener('click', async e => {
        e.preventDefault();
        const textToAnalyze = rtoTextarea.value.trim();
        if (!textToAnalyze) {
            showError('Please paste some text into the box to analyze.');
            return;
        }

        rtoAnalyzeBtn.textContent = 'Analyzing...';
        rtoAnalyzeBtn.disabled = true;
        rtoSuggestionsList.innerHTML = '<li>Analyzing...</li>';

        try {
            const data = await callOpenAiApi('readability_tone', textToAnalyze, urlInput.value || 'N/A');
            
            if (data.error) throw new Error(data.error);

            // Populate the stat cards
            rtoOverallTone.textContent = data.overall_tone || 'N/A';
            rtoGradeLevel.textContent = data.grade_level || 'N/A';
            rtoWordCount.textContent = data.word_count || 0;
            rtoPassiveVoice.textContent = `${data.passive_voice_percent || 0}%`;

            // Populate the suggestions list
            const suggestions = data.suggestions || [];
            if (suggestions.length > 0) {
                rtoSuggestionsList.innerHTML = suggestions.map(item => `
                    <div class="rto-suggestion-item">
                        <p><span class="original">${item.original}</span> → <span class="suggestion">${item.suggestion}</span></p>
                    </div>
                `).join('');
            } else {
                rtoSuggestionsList.innerHTML = '<li>No specific suggestions. Looks great!</li>';
            }

        } catch (err) {
            handleApiError('Readability & Tone', err);
            rtoSuggestionsList.innerHTML = '<li>Analysis failed. Please try again.</li>';
        } finally {
            rtoAnalyzeBtn.textContent = 'Analyze Pasted Text';
            rtoAnalyzeBtn.disabled = false;
        }
    });
...
<section class="maxw px-4 pb-10">
    ...
    <div class="tsi-grid">
      ...
      <div class="tsi-info-grid">
        <div class="tsi-info-item">
            ...
            <ul id="tsiInternalLinks"><li>Run analysis to see results.</li></ul>
        </div>
        <div class="tsi-info-item">
            ...
            <p id="tsiUrlSuggestion" style="margin-top:4px;">Run analysis to see results.</p>
        </div>
        ...
        <div class="tsi-info-item">
            ...
            <ul id="tsiAltTexts"><li>Run analysis to see results.</li></ul>
        </div>
        <div class="tsi-info-item">
            ...
            <div class="site-map-container" id="tsiSiteMap">Run analysis to see results.</div>
        </div>
      </div>
    </div>
    <div class="tsi-suggestions">
        <h4><span class="animated-icon">💡</span> Technical SEO Suggestions</h4>
        <ul id="tsiSuggestionsList"><li>Run analysis to see results.</li></ul>
    </div>
  </div>
  ...
  <!-- ✨ NEW: Readability & Tone Optimization ✨ -->
    <div style="display:flex; justify-content:center; align-items:center; gap:10px; margin-top:24px;">
        <span class="animated-icon" style="color:var(--green-1);">
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>
        </span>
        <h3 class="t-grad" style="font-weight:900;margin:0; font-size: 22px;">AI-Powered Readability & Tone</h3>
    </div>
    <div class="rto-card" id="readabilityToneCard">
        <div class="rto-grid">
            <!-- Input Column -->
            <div>
                <textarea id="rtoTextarea" class="rto-textarea" placeholder="The text from your analyzed URL will appear here automatically. You can also paste your own text."></textarea>
                <button id="rtoAnalyzeBtn" class="btn btn-green w-full mt-2">Analyze Pasted Text</button>
            </div>
            <!-- Results Column -->
            <div>
                <div class="rto-results-grid">
                    <div class="rto-stat-card">
                        <div class="rto-stat-label">Overall Tone</div>
                        <div id="rtoOverallTone" class="rto-stat-value">—</div>
                    </div>
                    <div class="rto-stat-card">
                        <div class="rto-stat-label">Grade Level</div>
                        <div id="rtoGradeLevel" class="rto-stat-value">—</div>
                    </div>
                    <div class="rto-stat-card">
                        <div class="rto-stat-label">Word Count</div>
                        <div id="rtoWordCount" class="rto-stat-value">0</div>
                    </div>
                    <div class="rto-stat-card">
                        <div class="rto-stat-label">Passive Voice</div>
                        <div id="rtoPassiveVoice" class="rto-stat-value">0%</div>
                    </div>
                </div>
                <h4 class="ki-item-title" style="margin-bottom: 8px;"><span class="animated-icon">💡</span>AI Suggestions</h4>
                <div id="rtoSuggestionsList" class="rto-suggestions-list">
  ...
  <div class="card meta-card" style="margin-top:24px;">
    ...
    <div class="space-y-3">
        <div class="cat-card">
            ...
            <div id="titleVal" style="color:var(--ink); margin-top:4px;">Run analysis to see results.</div>
        </div>
        <div class="cat-card">
            <div class="cat-card-title">Meta Description</div>
            <div id="metaVal" style="color:var(--ink); margin-top:4px;">Run analysis to see results.</div>
        </div>
        <div class="cat-card">
            <div class="cat-card-title">Heading Map (H1-H4)</div>
            <div id="headingMap" class="space-y-2" style="margin-top:8px;">Run analysis to see results.</div>
        </div>
    </div>
  </div>
  ...
  <div class="ground-slab">
    ...
    <div id="cats" class="grid grid-cols-2 gap-3"></div>
  </div>
